...
class Network( Topo ):
    def build( self ):
        ...
        h1= self.addHost('h1', cpu=0.2,ip='192.168.0.2/28')
        h2= self.addHost('h2', cpu=0.2,ip='192.168.0.3/28')
        h3= self.addHost('h3', cpu=0.2, ip='192.168.0.4/28')
        # h4= self.addHost('h4', cpu=0.2, ip='192.168.0.5/28')
        ...
        for d, s in [ (h0, s0), (h1, s1), ]:
            self.addLink( d, s, bw=10, delay='10ms', loss=1, use_htb=True )
        for d, s in [ (h2, s2), (h3, s3) ]:
            self.addLink( d, s, bw=20, delay='20ms', loss=1, use_htb=True )
        
        # Switch to Swtich 
        for d, s in [ (s0, s1), (s0, s2), (s0, s3) ]:
            self.addLink( d, s, bw=30, loss=3, delay='5ms', use_htb=True )


        # Switch to Switch
        for d, s in [ (s1, s2), (s1, s3), (s1, s3), (s2, s3) ]:
            self.addLink( d, s, bw=30, loss=2, delay='5ms', use_htb=True )
...
def run():
    ...
    net = Mininet(topo=Network(), host=CPULimitedHost, link=TCLink, autoStaticArp=True, switch=OVSSwitch)
    net.addController('ryu-0', controller=RemoteController, ip='0.0.0.0', port=6633)
